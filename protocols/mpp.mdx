...
The agent signs an on-chain authority tx, then re-submits with `Authorization: Payment`. AlgoVoi confirms the tx is on-chain (typically within 60 seconds), activates the subscription, and returns a `subscriptionId`.
