...
- **UK**: holding pre-paid customer balances triggers FCA safeguarding obligations under the Electronic Money Regulations 2011 if the activity meets the e-money definition. AlgoVoi operates the APM balance as a **segregated client-money account**, not commingled with operating funds, and is in the process of obtaining the relevant authorisation (status: in preparation — see [Compliance](/compliance)).
...
Until full authorisation lands, APM balances are capped per-mandate (default £100, configurable per-operator) to keep total client-money exposure within the small-issuer threshold. Operators offering APM to customers should disclose the safeguarding status — boilerplate copy is in `/legal/mandate-terms`.
...
- **Top-ups are not refundable** once spent — when `balance_minor` is debited and a merchant payout `tx_id` is confirmed, the funds have left the system on-chain and can't be clawed back. **Unspent** balances are refundable on mandate closure, subject to identity verification.
...
There is no chargeback model. Mandate top-ups are non-refundable once balance is debited to settle a merchant payout (the merchant has been paid on-chain — there's nothing to claw back). **Unspent** balance is refundable on mandate closure subject to identity verification. Operators should set conservative `period_cap_minor` and `ttl_secs` for new accounts.
