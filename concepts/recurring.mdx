...
4. **Activation** — When the credential is posted synchronously, AlgoVoi verifies the authority tx inline (via `verify_authority_tx` per-chain) and returns `status: "active"` with a base64url `subscriptionId` and a `Payment-Receipt` header. If the authority tx isn't yet confirmed on chain at credential-post time, the subscription is created in `pending` status and the activation reaper transitions it to `active` once the tx lands (typically within 60 seconds).
