...
Tenant secrets (API keys, webhook secrets, OAuth tokens) are stored encrypted with Fernet. The gateway holds the Fernet key in env. Rotation is multi-key (MultiFernet) to allow zero-downtime rolls.
