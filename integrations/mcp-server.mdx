...
The AlgoVoi MCP server (v1.4.2 Python / v1.4.1 TypeScript) lets any [Model Context Protocol](https://modelcontextprotocol.io) client call AlgoVoi's gateway as if its tools were native. An assistant can create payment links, verify transactions, probe MPP endpoints, screen recipients, generate protocol challenges, call payment-gated A2A agents, and manage recurring payment authorities — all from a normal conversation.
...
| `list_networks` | Return all supported chains and assets (all 7 chains, 25 network keys) |
...
| `try_mpp_endpoint` | Probe any MPP-protected URL; decode the `WWW-Authenticate: Payment` challenge and return the `accepts[]` array — mirrors `npx agentcash try <url>` |
...
| --- | --- | --- |
| **Python** `algovoi-mcp` | [pypi.org/project/algovoi-mcp/1.4.2](https://pypi.org/project/algovoi-mcp/1.4.2/) | `uvx algovoi-mcp==1.4.2` |
| **TypeScript** `@algovoi/mcp-server` | [npmjs.com/package/@algovoi/mcp-server](https://www.npmjs.com/package/@algovoi/mcp-server) | `npx -y @algovoi/mcp-server@1.4.1` |

Both expose the same 25 tools across all 7 chains. Functional behaviour is identical.
...
```json
{
  "mcpServers": {
    "algovoi": {
      "command": "npx",
      "args": ["-y", "@algovoi/mcp-server@1.4.1"],
      "env": {
        "ALGOVOI_API_KEY": "algv_…",
        "ALGOVOI_TENANT_ID": "your-tenant-id",
        "ALGOVOI_PAYOUT_ALGORAND": "your-algorand-address"
